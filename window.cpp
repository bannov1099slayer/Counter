...
void Window::init()
{
    ...
    third  = new QGroupBox(tr("Счётчики правильных и неправильных ответов"),this);
...
//--------------------------------------------------------------------------------------------------------------------
// Смена фона лайнэдитов для индикации смены числа кликов, которые нужно прокликать
//--------------------------------------------------------------------------------------------------------------------
void Window::setRandomBackgroundColor()
{
    t = t + 1 + rand()%6;

    if(t == 1) le2->setStyleSheet("color: blue; background-color: magenta");

    if(t == 2) le2->setStyleSheet("color: blue; background-color: yellow");

    if(t == 3) le2->setStyleSheet("color: blue; background-color: green");

    if(t == 4) le2->setStyleSheet("color: black; background-color: cyan");

    if(t == 5) le2->setStyleSheet("color: blue; background-color: red");

    if(t == 6) le2->setStyleSheet("color: blue; background-color: gray");
...
//--------------------------------------------------------------------------------------------------------------------
// Проверка совпадения числа кликов
//--------------------------------------------------------------------------------------------------------------------
void Window::check()
{
    qDebug() << "check";
...
//--------------------------------------------------------------------------------------------------------------------
// Чистит форму и обнуляет счётчики
//--------------------------------------------------------------------------------------------------------------------
void Window::cleaner()
{
    qDebug() << "clear";
...
//--------------------------------------------------------------------------------------------------------------------
// Вызов диалогового окна при попытке закрыть основное окно
//--------------------------------------------------------------------------------------------------------------------
